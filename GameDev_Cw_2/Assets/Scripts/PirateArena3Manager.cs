...
public class PirateArena3Manager : MonoBehaviour
{
    ...
    void spawnEnemies()
    {
        if (timer > 0)
        {
            ...
            GameObject enemy;
            enemy = Instantiate(boatPiratePrefab, new Vector3(spawnPoint1.x, spawnPoint1.y, 0), Quaternion.identity);
            enemy.GetComponent<EnemyAI>().target = playerInstance;
            enemy.GetComponent<EnemyAttack>().player = playerInstance;
            enemy = Instantiate(boatPiratePrefab, new Vector3(spawnPoint2.x, spawnPoint2.y, 0), Quaternion.identity);
            enemy.GetComponent<EnemyAI>().target = playerInstance;
            enemy.GetComponent<EnemyAttack>().player = playerInstance;
        }
    }
